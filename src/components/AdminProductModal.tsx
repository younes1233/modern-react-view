...
// Update summary for this version:
// - Replaced top-level `available_countries` with a single numeric `country_id` from AuthContext.
// - No UI for country selection; we silently set `country_id` from context and send it in the payload.
// - REMOVED the global "Available Countries" input.
// - REMOVED "Country ID" and "VAT %" fields from both Product Prices and Variant Prices UI.
// - Those removed fields are not sent in the payload anywhere.
...
interface AdminProductModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (product: CreateProductData & { country_id?: number }) => void;
 ...
 * The form captures all fields required by the Product API, with `country_id` (number) sent instead of `available_countries`.
 */
export function AdminProductModal({
  ...
  // Variants: removed available_countries + removed country_id/vat from prices
  ...
  useEffect(() => {
    if (product && mode === "edit") {
      setFormData({
        ...
        variants: [],
        // Prefer context if available; otherwise leave undefined.
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Build product prices (ONLY net_price & cost)
    ...
    if (formData.has_variants && variantEntries.length > 0) {
      variantsPayload = variantEntries.map((variant) => {
        ...
        return {
          image: variant.image,
          variations,
          product_variant_prices: variantPricesParsed.length > 0 ? (variantPricesParsed as any) : undefined,
    ...
    const payload: CreateProductData & { country_id?: number } = {
      ...(rest as CreateProductData),
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Select value={String(formData.category_id)} onValueChange={(v) => updateField("category_id", parseInt(v, 10))}>
                <SelectTrigger>
                  <SelectValue>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => updateField("shelf_id", value ? parseInt(value, 10) : undefined)}
                >
                  <SelectTrigger>
          ...
          {/* Product Prices (NO Country ID / VAT %) */}
            ...
            {specifications.map((spec, index) => (
              <div key={spec.id} className="grid grid-cols-3 gap-2 items-center">
                ...
                <Input
                  ...
                  placeholder="Value"
                />
                <Button type="button" variant="destructive" size="sm" onClick={() => removeSpecification(index)} disabled={index < 4}>
                  Remove
                </Button>
              </div>
            ))}
              ...
              {variantEntries.map((variant) => (
                <div key={variant.id} className="border p-4 rounded space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Button type="button" variant="secondary" size="sm" onClick={() => addVariantPrice(variant.id)}>
                      Add Price
                    </Button>
                    {variant.variantPrices.map((p, idx) => (
                      <div key={idx} className="grid grid-cols-3 gap-2 items-center">
                        ...
                        <Input
                          ...
                          placeholder="Cost"
                        />
                        <Button type="button" variant="destructive" size="sm" onClick={() => removeVariantPrice(variant.id, idx)}>
                          Remove
                        </Button>
                      </div>
                    ))}
                  ...
                  <div className="space-y-2">
                    ...
                    <Button type="button" variant="secondary" size="sm" onClick={() => addVariantSpec(variant.id)}>
                      Add Spec
                    </Button>
                    {variant.variantSpecs.map((spec, idx) => (
                      <div key={idx} className="grid grid-cols-3 gap-2 items-center">
                        ...
                        <Input
                          ...
                          placeholder="Value"
                        />
                        <Button type="button" variant="destructive" size="sm" onClick={() => removeVariantSpec(variant.id, idx)}>
                          Remove
                        </Button>
                      </div>
                    ))}
